...
describe('application reducer', () => {
  ...
  describe('updateBlockNumber', () => {
    it('updates block number', () => {
      store.dispatch(updateBlockNumber({ chainId: ChainId.CYPHER_MAINNET, blockNumber: 4 }))
      expect(store.getState().blockNumber[ChainId.CYPHER_MAINNET]).toEqual(4)
    })
    it('no op if late', () => {
      store.dispatch(updateBlockNumber({ chainId: ChainId.CYPHER_MAINNET, blockNumber: 2 }))
      expect(store.getState().blockNumber[ChainId.CYPHER_MAINNET]).toEqual(3)
    })
