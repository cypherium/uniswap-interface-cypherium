...
describe('transaction reducer', () => {
  ...
  describe('addTransaction', () => {
    it('adds the transaction', () => {
      ...
      store.dispatch(
        addTransaction({
          chainId: ChainId.CYPHER_MAINNET,
          summary: 'hello world',
          hash: '0x0',
      ...
      expect(txs[ChainId.CYPHER_MAINNET]).toBeTruthy()
      expect(txs[ChainId.CYPHER_MAINNET]?.['0x0']).toBeTruthy()
      const tx = txs[ChainId.CYPHER_MAINNET]?.['0x0']
  ...
  describe('clearAllTransactions', () => {
    it('removes all transactions for the chain', () => {
      ...
      expect(Object.keys(store.getState())).toEqual([String(ChainId.CYPHER_MAINNET), String(ChainId.RINKEBY)])
      expect(Object.keys(store.getState()[ChainId.CYPHER_MAINNET] ?? {})).toEqual(['0x0'])
      expect(Object.keys(store.getState()[ChainId.RINKEBY] ?? {})).toEqual(['0x1'])
      store.dispatch(clearAllTransactions({ chainId: ChainId.CYPHER_MAINNET }))
      expect(Object.keys(store.getState())).toHaveLength(2)
      expect(Object.keys(store.getState())).toEqual([String(ChainId.CYPHER_MAINNET), String(ChainId.RINKEBY)])
      expect(Object.keys(store.getState()[ChainId.CYPHER_MAINNET] ?? {})).toEqual([])
      expect(Object.keys(store.getState()[ChainId.RINKEBY] ?? {})).toEqual(['0x1'])
    })
  })
})
